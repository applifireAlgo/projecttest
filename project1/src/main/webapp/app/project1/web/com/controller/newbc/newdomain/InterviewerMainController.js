Ext.define('Project1.project1.web.com.controller.newbc.newdomain.InterviewerMainController', {
     extend: 'Project1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.InterviewerMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#InterviewerTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#InterviewerForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#InterviewerGrid');
               var tree = this.view.down('#InterviewerTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#InterviewerForm');
          var grid = this.view.down('#InterviewerGrid');
          var tree = this.view.down('#InterviewerTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#InterviewerGrid');
               ...
               var formPanel = this.getView().up().down('#InterviewerForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#InterviewerTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#InterviewerForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#InterviewerGrid').store;
          gridStore.removeAll();
