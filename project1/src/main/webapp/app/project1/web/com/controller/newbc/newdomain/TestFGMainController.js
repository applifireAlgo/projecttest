Ext.define('Project1.project1.web.com.controller.newbc.newdomain.TestFGMainController', {
     extend: 'Project1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TestFGMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#TestFGTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#TestFGForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#TestFGGrid');
               var tree = this.view.down('#TestFGTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#TestFGForm');
          var grid = this.view.down('#TestFGGrid');
          var tree = this.view.down('#TestFGTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#TestFGGrid');
               ...
               var formPanel = this.getView().up().down('#TestFGForm');
               jsonData = {
                    'findKey': primaryKey
               }
               ...
               var FGPTGrid = formPanel.down('#FGPTGrid');
               FGPTGrid.store.loadData(data.fGPT);
               FGPTGrid.setSelection(0);
          }
     },
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#TestFGTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestFGForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestFGGrid').store;
          gridStore.removeAll();
     ...
     addFGPT: function(but) {
          var formFGPT = but.up().up().down('form');
          var gridFGPT = but.up().up().down('#FGPTGrid');
          if (formFGPT.isValid()) {
               var values = this.getData(formFGPT);
               values = values[gridFGPT.bindable];
               if (gridFGPT.selection) {
                    gridFGPT.selection.data = values;
               } else {
                    gridFGPT.getStore().add(values);
               }
               gridFGPT.reconfigure();
               formFGPT.reset();
               gridFGPT.setSelection();
          }
     },
     onFGPTGridItemClick: function(cellModel, record, rowIndex, columnIndex, eOpts, comp) {
          var form;
          if (comp != null) {
               form = comp.up();
          } else {
               form = this.view.down('#FGPTForm');
          }
     ...
     resetForm: function(but) {
          ...
          tree.setSelection();
          grid.setSelection();
          var gridFGPT = this.view.down('#FGPTGrid');
          gridFGPT.getStore().removeAll();
          gridFGPT.reconfigure();
     },
     saveForm: function(but, evt) {
          var form = but.up('form');
          var FGPTGrid = this.view.down('#FGPTGrid');
          var FGPTForm = this.view.down('#FGPTForm');
          if (!(FGPTForm.isValid() || FGPTGrid.store.getCount() != 0)) {
               return;
          }
          this.addFGPT(FGPTForm.down('button'));
          FGPTForm.reset();
