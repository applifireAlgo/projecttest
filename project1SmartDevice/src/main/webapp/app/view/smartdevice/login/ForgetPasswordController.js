Ext.define('Project1.view.smartdevice.login.ForgetPasswordController',
		{
				...
				if (this.getView().down('#secQuestion').isValid()
					...
					Ext.Ajax.request({
						...
						success : function(response, sender) {
							...
							container.removeAll();
							container.add(Ext.create('Project1.view.smartdevice.login.Login'));
							sender.entMask.hide();
						},
			...
			onCancelClick : function(btn, opts) {
				...
				container.removeAll();
				container.add(Ext
						.create('Project1.view.smartdevice.login.Login'));
			}
});